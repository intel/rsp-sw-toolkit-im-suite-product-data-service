...
// Modified code to merge gojsonpointer and gojsonreference into package gojsonschema,
//		changing paths and references accordingly
...
func (d *Schema) parseSchema(documentNode interface{}, currentSchema *subSchema) error {
	...
	if existsMapKey(m, KEY_SCHEMA) {
		...
		schemaReference, err := NewJsonReference(schemaRef)
	...
	if k, ok := m[KEY_REF].(string); ok {
		...
		jsonReference, err := NewJsonReference(k)
		if err != nil {
			return err
		}
